...
namespace Application.HumanResources.Commands.CreateHumanResource
{
    public class CreateHumanResourceCommandValidator : AbstractValidator<CreateHumanResourceCommand>
    {
        ...
        protected bool BeUniqueEmail(string email)
        {
            var result = _repository.CheckEmailExists(email?.Trim().ToLower());

            return result is null;
        }

        protected bool BeUniqueEmployeeNumber(int employeeNumber)
        {
            var result = _repository.CheckEmployeeNumberExists(employeeNumber);
